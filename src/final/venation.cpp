    ...
    int maxEdgesToShow)
{
    ...
    // materials
    SceneMaterial woodMat{};
    woodMat.cAmbient  = glm::vec4(0.10f, 0.06f, 0.03f, 1.f);
    woodMat.cDiffuse  = glm::vec4(0.55f, 0.35f, 0.20f, 1.f);
    woodMat.cSpecular = glm::vec4(0.08f, 0.08f, 0.08f, 1.f);
    woodMat.shininess = 10.f;
    ...
    float maxFlow = 1.f;
    for (float f : flow) maxFlow = std::max(maxFlow, f);

    constexpr float radiusScaleMin = 0.25f;
    constexpr float radiusScaleMax = 2.0f;
    ...
    // add floor
    {
        ...
        glm::vec3 floorCenter(
            0.5f*(p.domainMin.x + p.domainMax.x),
            p.domainMin.y - 0.5f*floorThickness,
            0.5f*(p.domainMin.z + p.domainMax.z)
            );
        ...
        glm::mat4 S = glm::scale(glm::mat4(1.f),
                                 glm::vec3(width, floorThickness, depth));
        glm::mat4 T = glm::translate(glm::mat4(1.f), floorCenter);
    ...
    // first n edges
    int count = std::min(maxEdgesToShow, (int)edges.size());
    ...
    for (int ei = 0; ei < count; ++ei) {
        ...
        // flow based r
        float f = flow[e.from];
        float t = std::log(1 + f) / std::log(1 + maxFlow);
        float radiusScale = radiusScaleMin + (radiusScaleMax - radiusScaleMin)*t;
        float r = std::max(0.02f * baseR, baseR * radiusScale);
